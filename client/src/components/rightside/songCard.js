...
import { isSongPlayingReducer, songReducer, searchReducer } from "../../redux/reducer";
import { MdDelete } from "react-icons/md";
import { BiHeart } from "react-icons/bi";
import { deleteSongById, editFavorite, editSongName } from '../../redux/saga/saga'
...
const MainDiv3 = styled.div`
    /* absolute inset-0 justify-center items-center bg-black bg-opacity-50 group-hover:flex ${activeSong?.title === song.title ? 'flex bg-black bg-opacity-70' : 'hidden'} */
  position: absolute;
  width: auto;
  height: auto;
  top: 3rem;
...
const SongCard = () => {
  ...
  const editName = (id, oldName) => {
    ...
    name && editSongName(id, name).then((res) => {
      dispatch(setAllSongs())
    });
  }
    ...
    <>
      ...
      {data ? data.filter((data) =>
        ...
        <MainDiv key={songs._id} className="song-card-hover">
          <MainDiv2 className="song-card-hover" onClick={() => setCurrentPlaySong(i)}>
          ...
          <FavoriteDiv className="favor-div">
            ...
            {songs.favorite ? <FavoriteTrue onClick={() => editFavorite(songs._id, !songs.favorite)} /> : <FavoriteIcon onClick={() => editFavorite(songs._id, !songs.favorite)} />}
